...
struct BrowserWebContentView: View {
    ...
    @Query(sort: \PasswordItem.siteName) private var passwordItems: [PasswordItem]
            ...
            if tab.isWebViewReady {
                if tab.hasNavigationError, let error = tab.navigationError {
                    ...
                    ZStack(alignment: .topTrailing) {
                        ...
                        if tab.passwordFieldFocused, let host = tab.url.host {
                            PasswordFieldPopover(
                                host: host,
                                tab: tab,
                                items: passwordItems,
                                fieldRect: tab.passwordFieldRect
                            )
                            .zIndex(1001)
                        }
                    }
