...
class Tab: ObservableObject, Identifiable {
    ...
    var savedURL: URL?
    var title: String
    var customName: String? // Custom name set by user, takes precedence over title
    ...
    /// The title to display in the UI. Returns customName if set, otherwise returns title.
    ...
    func setupNavigationDelegate() {
        ...
        delegate.onTitleChange = { [weak self] title in
            DispatchQueue.main.async {
                ...
                if let title, !title.isEmpty, self.customName == nil {
                    self.title = title
