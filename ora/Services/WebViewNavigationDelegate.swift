...
private let logger = Logger(subsystem: "eu.flareapps.prism", category: "WebViewNavigationDelegate")
...
// Password field focus detection
(function () {
    if (window.__oraPasswordFocusInstalled) return;
    window.__oraPasswordFocusInstalled = true;
    ...
    function isPasswordField(input) {
        ...
        // Check if it's a username/email field (often near password fields)
        if (type === 'email' || type === 'text') {
            // Common patterns for username fields
            const usernamePatterns = /user|login|email|mail|account|name/i;
            if (usernamePatterns.test(name) || usernamePatterns.test(id)) {
                return true;
            }
        }
    ...
    function getFieldRect(input) {
        const rect = input.getBoundingClientRect();
        // Return viewport-relative coordinates (not document-relative)
        return {
            x: rect.left,
            y: rect.top,
            width: rect.width,
    ...
    function notifyFieldFocus(input) {
        if (!isPasswordField(input)) return;
        
        try {
            const rect = getFieldRect(input);
    ...
    document.addEventListener('focusout', (e) => {
        ...
        setTimeout(() => {
            ...
            if (!active || !isPasswordField(active)) {
                notifyFieldBlur();
            }
        }, 100);
    }, true);
...
class WebViewNavigationDelegate: NSObject, WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        if !isDownloadNavigation {
            ...
            injectExtensionScripts(into: webView)
    ...
    /// Inject extension scripts for the current page
    private func injectExtensionScripts(into webView: WKWebView) {
