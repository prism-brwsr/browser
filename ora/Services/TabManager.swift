...
class TabManager: ObservableObject {
    ...
    func renameTab(_ tab: Tab, customName: String?) {
        tab.customName = customName?.trimmingCharacters(in: .whitespacesAndNewlines)
        if tab.customName?.isEmpty == true {
            tab.customName = nil
        }
        try? modelContext.save()
    }
    ...
    func createContainer(name: String = "Default", emoji: String = "👤") -> TabContainer {
        ...
        newContainer.sortIndex = nextContainerSortIndex()
    ...
    func reorderContainers(draggedID: UUID, targetID: UUID?) {
        var orderedContainers = containersSortedByDisplayIndex()
        guard let fromIndex = orderedContainers.firstIndex(where: { $0.id == draggedID }) else { return }

        let movingContainer = orderedContainers.remove(at: fromIndex)

        if let targetID, let originalTargetIndex = orderedContainers.firstIndex(where: { $0.id == targetID }) {
            var destinationIndex = originalTargetIndex
            if fromIndex < originalTargetIndex {
                destinationIndex -= 1
            }
            orderedContainers.insert(movingContainer, at: max(destinationIndex, 0))
        } else {
            orderedContainers.append(movingContainer)
        }

        var didChange = false
        for (index, container) in orderedContainers.enumerated() {
            if container.sortIndex != index {
    ...
    func persistContainerOrder(_ orderedContainers: [TabContainer]) {
        var didChange = false
        for (index, container) in orderedContainers.enumerated() {
            if container.sortIndex != index {
                container.sortIndex = index
    ...
    private func containersSortedByDisplayIndex() -> [TabContainer] {
        do {
    ...
    private func nextContainerSortIndex() -> Int {
        let maxIndex = containersSortedByDisplayIndex().map(\.sortIndex).max() ?? -1
        return maxIndex + 1
    }

    private func ensureContainerSortIndexes() {
        let containers = containersSortedByDisplayIndex()
        var didChange = false
...
final class TabSearchingService: TabSearchingProviding {
            ...
            predicate = #Predicate { tab in
                (
                    tab.urlString.localizedStandardContains(trimmedText) ||
                        ...
                        (tab.customName != nil && tab.customName!.localizedStandardContains(trimmedText))
    ...
    private func scoreMatch(_ tab: Tab, text: String) -> Int {
        ...
        let customName = tab.customName?.lowercased() ?? ""
        ...
        // Prioritize customName matches slightly over title matches
        let customNameScore = score(customName)
        let titleScore = score(title)
        let urlScore = score(url)
        
        // If customName matches, give it priority
        if customNameScore > 0 {
            return max(customNameScore + 5, titleScore, urlScore)
        }
        
        return max(titleScore, urlScore)
    }
}
